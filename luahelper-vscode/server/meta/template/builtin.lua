...
---@class nil:any @The type `nil` has one single value `nil`, whose main property is to be different from any other value. [`View online doc`](https://www.lua.org/manual/5.1/manual.html#2)  |  [`View local doc`](command:extension.lua.doc?["en-us/51/manual.html/2"])

---@class boolean: any @The type `boolean` has two values, `false` and `true`. [`View online doc`](https://www.lua.org/manual/5.1/manual.html#2)  |  [`View local doc`](command:extension.lua.doc?["en-us/51/manual.html/2"])

---@class number:any @The type `number` uses two internal representations, or two subtypes, one called *integer* and the other called *float*. [`View online doc`](https://www.lua.org/manual/5.1/manual.html#2)  |  [`View local doc`](command:extension.lua.doc?["en-us/51/manual.html/2"])

---@alias integer number @integer numbers

---@class thread: any @The type *thread* represents independent threads of execution and it is used to implement coroutines. Lua threads are not related to operating-system threads. Lua supports coroutines on all systems, even thosethat do not support threads natively. [`View online doc`](https://www.lua.org/manual/5.1/manual.html#2)  |  [`View local doc`](command:extension.lua.doc?["en-us/51/manual.html/2"])

---@class table: any @The type *table* implements associative arrays, that is, arrays that can have as indices not only numbers, but any Lua value except **nil** and NaN.(*Not a Number* is a special floating-point value used by the IEEE 754 standard to represent undefined or unrepresentable numerical results, such as `0/0`.) Tables can be heterogeneous; that is, they can contain values of all types (except **nil**). Any key with value **nil** is not considered part oft he table. Conversely, any key that is not part of a table has an a ssociated value **nil**. [`View online doc`](https://www.lua.org/manual/5.1/manual.html#2)  |  [`View local doc`](command:extension.lua.doc?["en-us/51/manual.html/2"])
